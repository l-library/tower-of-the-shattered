...
Scene* MainGameScene::createWithMap(const std::string& mapFile, cocos2d::Vec2 pos)
{
    MainGameScene* pRet = new(std::nothrow) MainGameScene();
...
bool MainGameScene::init()
{
    ...
    if (gameManager->hasPlayer()) {
        ...
        CCLOG("从GameManager获取保存的Player");

        // 重置Player状态
        resetPlayerForNewScene(_player);
