...
void EnemyBase::setupPhysicsBody(float width, float height)
{
    ...
    // 设置物理体属性
    physicsBody_->setDynamic(true); // 敌人是动态的，可以移动
    physicsBody_->setMass(1.0f); // 设置质量
    physicsBody_->setRotationEnable(false); // 禁止旋转
    physicsBody_->setCategoryBitmask(0x02); // 敌人的碰撞类别
    physicsBody_->setContactTestBitmask(0xFFFFFFFF); // 接收所有碰撞检测
    physicsBody_->setCollisionBitmask(0xFFFFFFFF); // 参与所有碰撞反应
